...
* **Refactor (Módulo Scripts - Rama `dev-scripts`):** Modularización profunda de `Scrapping.py` aplicando Principio de Responsabilidad Única (SRP). Reestructuración del manejo de ciclo de vida de objetos (DataFrames) para una recolección de basura más eficiente.
